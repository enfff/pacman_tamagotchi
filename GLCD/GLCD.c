...
void LCD_DrawRectangleBorders (uint16_t X_start, uint16_t Y_start, uint16_t width, uint16_t height, uint16_t color)
{
	uint16_t i, j;
	
	for (j = 0; j < height; j++) {
		LCD_SetPoint(X_start, Y_start + j, color);
		LCD_SetPoint(X_start + width, Y_start + j, color);
	}
	
	for (i = 0; i < width; i++) {
		LCD_SetPoint(X_start + i, Y_start, color);
		LCD_SetPoint(X_start + i, Y_start + height, color);
	}
...
void GUI_Centered_Text(uint16_t X_start, uint16_t Y_start, uint16_t X_end, uint16_t Y_end, uint8_t *str, uint16_t Color, uint16_t bkColor)
{
	int Xpos, Ypos;
	int dX, dY;
	int str_length = 0;
	...
	while(curr_char != 0x00){			//dinamically finds string length
		curr_char = *str_cpy++;
		str_length += 1;	
	}
	...
	//TODO: FIX Ypos, CURRENTLY NOT WORKING
	Xpos = (int) ((dX - 8*str_length) / 2) + X_start;
	Ypos = (int) ((dY) - 16 / 2) + Y_start;
	
	GUI_Text(Xpos, Ypos, (uint8_t *)str, Color, bkColor);
}
